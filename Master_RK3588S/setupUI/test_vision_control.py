...
class VisionControlPlannerTest(unittest.TestCase):
    def test_conflicting_control_defaults_match_e7(self):
        ...
        self.assertEqual(config.car_avoid_speed_mps, 0.35)
        self.assertEqual(config.car_human_pass_speed_mps, 0.35)
        self.assertEqual(config.min_human_score, 0.40)
        self.assertEqual(config.min_car_score, 0.40)

    def test_human_and_car_control_thresholds_accept_exactly_point_four(self):
        planner = VisionControlPlanner(config=VisionControlConfig())
        image_shape = (480, 640, 3)
        bbox = [270, 300, 370, 470]

        low_car = [{"label": "Car", "score": 0.39, "bbox": bbox}]
        edge_car = [{"label": "Car", "score": 0.40, "bbox": bbox}]
        self.assertIsNone(planner._best_car(low_car, image_shape, 320.0))
        self.assertIsNotNone(planner._best_car(edge_car, image_shape, 320.0))

        low_human = [{"label": "Human", "score": 0.39, "bbox": bbox}]
        edge_human = [{"label": "Human", "score": 0.40, "bbox": bbox}]
        self.assertIsNone(planner._best_car_context_human(
            low_human, image_shape))
        self.assertIsNotNone(planner._best_car_context_human(
            edge_human, image_shape))
    ...
    def test_car_detection_masks_line_loss_response_during_avoid_hold(self):
        planner = VisionControlPlanner(config=_config())
    ...
    def test_car_human_pass_keeps_priority_when_line_is_lost(self):
        planner = VisionControlPlanner(config=_config())
        car = {
            "label": "Car", "score": 0.90,
            "bbox": [220.0, 260.0, 300.0, 420.0],
        ...
        human_on_avoid_side = {
            "label": "Human", "score": 0.90,
            "bbox": [380.0, 330.0, 440.0, 450.0],
        }
        human_after_crossing = {
            "label": "Human", "score": 0.90,
        ...
        planner.update(_raw_result_at(
            320.0, 430.0,
            detections=[car, human_on_avoid_side]), now=1.0)
        pass_command, _passed = planner.update(_raw_result_at(
            320.0, 430.0,
            detections=[car, human_after_crossing]), now=1.1)
        lost_command, lost = planner.update(
            _raw_result_at(slots=(), detections=[]), now=1.2)

        self.assertEqual(pass_command["state_cmd"], STATE_AVOID_HUMAN)
        self.assertEqual(lost_command["state_cmd"], STATE_AVOID_HUMAN)
        self.assertAlmostEqual(lost_command["target_speed"], 0.35)
        self.assertTrue(lost["line_loss_active"])
        self.assertTrue(lost["line_loss_response_masked"])
        self.assertEqual(
            lost["control_target"]["task_reason"],
    ...
    def test_car_in_path_uses_bounded_offset_and_avoid_speed(self):
        car = [{
            "label": "Car", "score": 0.90,
    ...
    def test_car_human_completed_pass_does_not_stop_again_for_same_person(self):
        planner = VisionControlPlanner(config=_config())
        car = {
            "label": "Car", "score": 0.90,
            "bbox": [220.0, 260.0, 300.0, 420.0],
        ...
        planner.update(_raw_result_at(
            320.0, 430.0,
            detections=[car, same_human_after_crossing]), now=1.1)
        command, debug = planner.update(_raw_result_at(
            320.0, 430.0,
            detections=[car, same_human_after_crossing]), now=3.2)
    ...
    def test_car_human_stop_restarts_after_five_second_dropout(self):
        planner = VisionControlPlanner(config=_config())
        car = {
            "label": "Car", "score": 0.90,
            "bbox": [220.0, 260.0, 300.0, 420.0],
        ...
        stopped, _ = planner.update(_raw_result_at(
            320.0, 430.0,
            detections=[car, human_on_avoid_side]), now=1.0)
        before_timeout, _ = planner.update(
            _raw_result_at(320.0, 430.0), now=5.99)
        restarted, debug = planner.update(
            _raw_result_at(320.0, 430.0), now=6.0)

        self.assertEqual(stopped["state_cmd"], STATE_AVOID_HUMAN)
        self.assertAlmostEqual(stopped["target_speed"], -0.10)
    ...
    def test_human_stop_latch_restarts_after_five_second_dropout(self):
        planner = VisionControlPlanner(config=_config())
        selected = {
            "points_xy": np.asarray(
                [[320.0, 60.0], [320.0, 460.0]], dtype=np.float32),
        ...
        missing_before_timeout = planner._human_action(
            [], selected, (480, 640, 3), 300.0, 320.0, 5.99)
        repeated_reverse = planner._update_human_stop_reverse(
            missing_before_timeout[2], 5.99)
        restart_after_timeout = planner._human_action(
            [], selected, (480, 640, 3), 300.0, 320.0, 6.0)

        self.assertEqual(stop[0], STATE_SAFE_STOP)
        self.assertTrue(first_reverse)
        self.assertFalse(repeated_reverse)
        self.assertEqual(missing_before_timeout[0], STATE_SAFE_STOP)
        self.assertEqual(missing_before_timeout[2], "human_absence_check")
        self.assertEqual(restart_after_timeout[0], STATE_AVOID_HUMAN)
        self.assertAlmostEqual(restart_after_timeout[1], 0.35)
        self.assertEqual(
            restart_after_timeout[2], "human_absence_restart")
