...
- 分割岔路后处理：粗扫描点发现双区域几何后，将采样间距从 `8 px` 临时加密到 `3 px`；由确认的左右内边界点拟合分割曲线，并用边界间隙收敛位置估计分叉点。双区域在上方时输出 `road_topology=FORK`，在下方时输出 `MERGE`，算法只切双区域一侧，共同区域保留给两条候选中线共享；拟合跨度/残差不合格则回退原行扫描候选。fork 分类器只用于可选道岔路和 stone 变道确认，不参与 `MERGE` 识别。MERGE 首次选择当前支路后锁定分支 side，候选仍可见期间只使用该分支 mask 的中线，不逐帧重选另一侧。
