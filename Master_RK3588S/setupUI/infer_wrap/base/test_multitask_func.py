...
class MultiTaskPostprocessTest(unittest.TestCase):
    ...
    def test_decodes_two_direct_paths_in_original_coordinates(self):
        outputs = list(self.make_outputs())
        ...
        self.assertEqual([item["role"] for item in result["paths"]],
                         ["left", "right"])
        self.assertEqual(result["paths"][0]["points_xy"].shape, (32, 2))
        self.assertAlmostEqual(
            float(result["paths"][0]["points_xy"][0, 0]),
            0.42 * 319, places=4)
        self.assertAlmostEqual(
            float(result["paths"][0]["points_xy"][0, 1]),
            0.95 * 239, places=4)
    ...
    def test_path_points_are_clipped_before_mapping(self):
        ...
        point = result["paths"][0]["points_xy"][0]
