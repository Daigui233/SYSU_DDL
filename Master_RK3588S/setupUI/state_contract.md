...
`STATE_COLLECT_GOLD` 和 `STATE_RECOVER_LINE` 都复用当前效果较稳定的
TC264D TRACK 电机/舵机控制参数。早期制作的其他任务枚举不再属于当前有效状态契约。
...
- `detections`：归一化检测对象，包含 `category`、`label`、`score`、`bbox`、`center`、`size`、`area_ratio`。
...
为了可读性和调试安全，第一阶段应远低于该上限。当前有效契约使用 8 个状态码：
`0` 空闲、`1..6` RK 任务状态、`7` 通用安全停车。新增状态前需要先定义清楚进入、
保持、退出和失效策略。
