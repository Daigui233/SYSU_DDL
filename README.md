...
- [代码架构](#代码架构)
- [踩过的坑](#踩过的坑)
- [项目历程回忆录](#项目历程回忆录实则度批斗大会以及车队心酸史)
- [补充说明](#补充说明)

## 后人先看这里（省流）

如果你是下一届准备接着做这个组，建议优先看以下几个部分：

1. `ArucoCalib-master/`
   定位系统，成熟度比较高，可以直接复用。

2. `Slave_TC264D/`
   下位机框架，可以直接作为下一届控制框架继续修改。

3. `PCB_Schematic/`
   硬件部分基本可以参考复用，但一定要先检查当年规则以及板卡接口是否变化。

4. `Master_RK3588S/`
   更建议参考整体思路，不建议直接照搬最终方案。
   上位机视觉方案迭代较多，而且受21届赛题和官方系统限制比较明显。

5. [训练模型的代码，需要跳转到另一个仓库链接，已经封装好可以直接上传傻瓜式点击训练了](https://github.com/cozylove-girl/X-SmartCar-AI-Model-Multitask)
   
***现在的模型其实效果还行，但是后面我们发现的主要问题一个是外包给外面标定数据质量较差（尽量自己标注吧，就算外包也要自己检查几遍），其次是我们很长一段时间都弄错了这个视角高度参数，导致采集的数据全部都是30cm的，而比赛要求是15cm左右，由于这种模型不需要特别强的泛化，没有经过特殊处理，于是数据质量的降低导致这个中线出来原先效果勉强还行，但是实则换回真正高度就只有近处的可以，远处成一团了，这也是数据集带来的问题，另外这个后处理也没有时间去做，写的比较简陋，可以点去另一个仓库详细看这方面的说明。***

如果重新开始做一届，我个人建议：
- 尽早把整车硬件搭完；
- 尽早解决定位和AR视频流；
- 4-5月前至少做出一个能够完整跑通流程的demo；
- 视觉方案不要等到比赛前一个月才大改；
- 尽早建立规范的Git版本管理和队内协作流程。

做模型的建议：
- 建议尽早弄完数据集，数据集的速度会影响模型出来的早晚，进而影响后面小车循迹的调参，在当下ai ***数据-算法-模型*** 架构下，数据的地位往往被人忽略，只有数据越早弄好才有时间进行后续处理（华工4w张数据起步，我们就几千张）
- 有余力应该了解一下其他方面的问题，模型是一个中期的任务，早期和晚期会显得无所事事，因为早期车没搭起来，没有数据，模型开不了工，晚期模型做好，也没什么工作，建议多了解一些别的，或者路径规划、数据结构这种的东西，比如路径规划就可以用图算法这类的
- 数据集应该多泛化一点，不同环境的，真实环境变化比数据增强好多了，负样本特别采集多一点来单独增强识别，然后这个数据集采集前先校对一下各类参数是否正常，尤其是相机相关参数

## 已知问题与后续建议

- 定位系统早期版本存在每帧自动曝光的问题，会导致帧率降低，建议固定曝光参数。
- 90° FOV 在实验室吊高环境下覆盖 5 m x 4 m 场地比较困难，建议考虑约 120° FOV 镜头。
- 上位机多模型并行推理曾出现明显性能问题，需要重新评估线程调度、CPU占用以及RKNN并行推理方案。
- 热力图中线方案仍有继续优化空间，最主要问题在于数据集规模和质量。
- 需要尽早测试完整通信链路，特别注意串口收发速率带来的系统延迟。
...
使用 **AprilTag**进行场地定位、坐标系建立以及视觉系统标定。主要用于AR视频流的融合而**不参与视觉元素的控制**。main分支下的定位系统由于早期全局摄像头每帧都要调整自动曝光导致帧数大概只有30帧左右，后期改进只自动曝光一次后保持曝光值就可以恢复到稳定60帧（可以自行修改）。考虑到实验室吊高问题，当时购买的是杰锐微通的全局快门摄像头，90° FOV，支持1080p，60 FPS.实际使用发现90° FOV虽然能够保证低畸变，但在实验室吊高环境下难以拍摄到**5x4m**的环境，**建议后续改用120° FOV的镜头**。该定位系统的使用方法也很简单，毕竟已经直接打包成Windows的exe执行文件了，放在了dist目录下。打开执行文件后勾选相应的几个选项，定位坐标使用电脑端与板卡端进行UDP通信进行发送，为了方便调车我还做了手柄遥控功能，调试可以用到的exe文件在下面也会提到。具体exe界面如图所示：
...
- **技术交流与合作**：坦白说，我们的基础工作做的不是特别好，对于资源的传承没有有效的传递，有经验的队员难以给到下一届的同学引路和避坑，这个时候就很需要**跨校交流与合作**。在21届我们也是和华工的同学有很多交流，从早期定位方案的犹豫不决，和华工交流后有了UWB方案和TAG两个备选，再到合作联调（其实有机会的话可以办那种校间的交流赛，作为建议）。总而言之，不要闭门造车，无论是问有经验的队员还是问友校的同学再或者是问相关方向的老师都是很好的选择，技术不是封闭的，交流才能更好进步，相信有技术积累的同学也不会拒绝交流的机会，所以可以和友校的同学们多交流。我们也是因为一些经验问题，调车的时候一直不太理想，直到最后才发现很长时间的延迟问题与串口的收发速率有关，所以一定要多交流。
...
- 大二上寒假，大概1月-2月左右，确定了参加智能模型组这个组别，一开始还不知道这个组别今年大改革，从原来的完全模型组（实体赛道）到了今年的智能模型组（AR赛道），一开始也没有确认完整的赛题还有给硬件板卡，所以一直止步不前，同时早在2月份根据官方所说本组在21届提供X车模的购买，也就是说我们参赛不需要怎么考虑硬件，只要准备好软件部分即可，所以我们早期规划就是等寒假返校后第一时间先购买物资然后做demo。但是智能车是一个很草台的比赛，经过部分人（说句实话我觉得这些人也不打这个组别）的“卓大您好”反馈说这个组今年太烧钱打不起之后，在3月左右禁止了售卖官方车模，也禁售了官方定位系统，全部让我们自己做，美其名曰降低成本（实则试错成本就上去了），此乃一变。
...
- 四月底装车后也是被定位系统卡了好一阵子，首先是代码仓库搭建，先把ADS环境配好之后搭建了代码仓库，开始搭建下位机代码框架，在此期间一部分队员负责定位系统的制作，在确认好使用TAG方案之后也是一开始选择了智能视觉的定位，买了逐飞的OpenMV作为定位的摄像头，但是效果不是很好。到了下位机框架搭建差不多的时候需要进行验证和调试了，还是被定位卡住进度，定位一直没有有效的效果，AR视频流中虚拟元素一直不动。直到6月初才下定决心换摄像头重做定位，效果立竿见影，也是在和华工的交流下就用了不到3天就解决了定位问题，终于能够开展下一步。

- 六月定位解决以后发现推理帧数一直很低，导致延迟 *（现在看来和串口收发速率也有点关系）*，但是好事是能跑，能够歪七扭八完赛了，也是士气大增（但是马上期末了有点力竭了）。然后就决定交流，找问题，看看是不是多个模型并行导致的CPU占用。在问了天大后发现他们用的是我们一直不看好的实例分割方案，也是用了一个简单的方法验证了，确实在改为单个模型后帧数有所提高，于是决定换方案，采用单模型推理的方式处理。六月底七月初的时候*度又开始发力，一口气整了两个活，一边说这个组不和区域赛一起举办，改为统一在上海举办，一边说改变赛题，取消原有的部分元素降低难度。距离比赛时间半个月了还没有确定比赛地点和比赛时间也是让我们备赛有点不知所措了。此乃二变。
...
## 补充说明
- 报销流程一般走小额报销，先自行垫付，后在[sycl系统](https://sycl.sysu.edu.cn/v/main)上进行申报，报销时提交**发票、支付记录、淘宝界面、excel统计表、sycl申报表**等，具体发票抬头应该会在群内发布，这一点在发布报销流程的时候也会再做进一步说明。此外，也可以走实验室经费，通过实验室采购进行，这个好处就是不需要先行垫付，但问题就是等待周期一般比较长，所以可以在放假期间走实验室采购。
- 推荐一下个人常用的工具，比如机场（其实能打开github看到应该都有渠道了），个人比较喜欢[三毛机场](https://xn--ehqx35aimmzwv.com/)，这个机场还是比较稳定性价比也比较高的，有这方面需求的还是可以的（公开场合可能不方便讲但在这里还是可以的吧hhh）
- 苯人是实验室垃圾佬，实验室有很多东西但是需要整理一下，基本的东西还是都有的，所以翻翻垃圾还是能找到不少好货（虽然这个可能不是什么好习惯），翻垃圾乐趣无穷啊
