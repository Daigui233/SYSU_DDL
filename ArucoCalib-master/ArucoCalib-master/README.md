...
- 程序首次同时读到 4 个四角 Tag 实例后会按画面中的左上、右上、左下、右下位置自动锁定标定，Tag ID 不参与标定且允许重名；锁定后固定 Tag 掉点或被碰到不会改变坐标系。需要重新标定时点击 `Reset Calibration` 并确认。
...
- 固定 Tag：任意 ID（包括 `ID 0`，也允许重复），尺寸 `0.20 m`；四角只按画面位置参与平面标定。
- 车载 Tag：仍为当前 `vehicle_id: 0`，ID 和朝向算法均未改变。若右下角固定 Tag 也是 `ID 0`，程序会保留锁定位置附近的那个固定实例，并把新增/移动的另一个 `ID 0` 作为车辆。
- 车载 Tag 解码已适度放宽：AprilTag 36h11 使用 `max_correction_bits: 5` 与 `errorCorrectionRate: 0.6`，可容忍约 3 个错误数据位；车辆仍为单帧识别到 `ID 0` 即接受。
- 四角锁定后，每个相机帧都先同步展开为统一尺度的场地俯视图；正常帧只执行一次 Tag 检测。只有原始矫正图无法筛出车载 ID0 时，才在上一帧车辆附近同步执行高光/暗部 Gamma 与 CLAHE 补偿检测，连续丢失 3 帧后扩大到全场。补偿结果只接收车辆 ID，并与固定 ID0 空间去重；没有跨帧等待或旧位置代替新位置。
- 场地：`4 m × 3 m`。
- 检测参数：当前使用稳定配置；若远端固定 Tag 仍不稳，优先增大 Tag 或改善安装/光照。
- 右下角基准点：`(X=500, Z=400)` mm；底边 `Z=400` mm，上边 `Z=3400` mm，`+X` 向左，`+Z` 向上。
- Yaw：`0° -> +X`，`+90° -> +Z`。
- 在场地图俯视方向：
  `0°` 向左、`+90°` 向上、`-90°` 向下、`±180°` 向右。
- `pose_filter` 对输出坐标和 Yaw 做轻量滤波；左上角 HUD、右上角 `UDP trajectory` 和 UDP 发送使用同一份滤波后的定位。

当前四角临时中心坐标（配置中的数字键仅为兼容旧配置，不再要求等于实际 Tag ID）：
...
正式安装后必须将 `world_coordinates` 的四个坐标值改为现场实测的四角 Tag 中心坐标；检测时只使用这些坐标值，不用数字键匹配 Tag ID。建议先让四个固定 Tag 完成锁定，再把车载 `ID 0` 放入画面。相机和固定 Tag 移动后应重新标定。
